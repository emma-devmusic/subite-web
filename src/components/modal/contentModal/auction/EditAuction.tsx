...
import { updateAuction } from "@/store/slices/auctionSlice";
...
export const EditAuction = () => {
    ...
    const [init_date, setInit_date] = useState<Date | string>(auctionSelected.init_date)
    const [end_date, setEnd_date] = useState<Date | string>(auctionSelected.end_date)
    const [values, handleInputChange] = useAppForm({
        price: productSelected.product_variations[0].price,
        bid_amount: auctionSelected.bid_amount,
    })

    useEffect(() => {
        setEnd_date(dayjs(init_date).add(1, 'day').format('YYYY-MM-DDTHH:mm'))
    },[init_date])
    ...
    const handleNewAuction = () => {
        ...
        dispatch(updateAuction({
            ...
            init_date: dayjs(init_date).format('YYYY-MM-DDTHH:mm:ss[Z]'),
            end_date: dayjs(end_date).format('YYYY-MM-DDTHH:mm:ss[Z]'),
            ...values
        }))
        // console.log(dayjs(init_date).format('YYYY-MM-DDTHH:mm:ss[Z]'))
        // console.log(dayjs(end_date).format('YYYY-MM-DDTHH:mm:ss[Z]'))
    }
    ...
    return (
        <form action="" >
            <div className="bg-white px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
                <div>
                    <h6 className="text-center mb-6">Subastar el producto <span className="text-cyan-700">{productSelected.name}</span></h6>
                </div>
