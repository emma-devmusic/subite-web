...
export const TableAuctionsRow = (product: ItemProductSearchResponse) => {
    ...
    const handleUserDelete = () => {
        Swal.fire({
            ...
            text: "¿Estás seguro/a que deseas eliminar esta subasta?. Este proceso no puede deshacerse.",
            icon: "warning",
            showCancelButton: true,
            confirmButtonColor: "#d33",
            cancelButtonColor: "#3085d6",
            confirmButtonText: "Si, Eliminar Subasta",
            cancelButtonText: 'Cancelar'
        }).then((result) => {
            if (result.isConfirmed) {

                auction && dispatch(deleteAuction({ id: auction.id, auctionTab }))
            }
        });
    }
    ...
    const handleEditProduct = () => {
        ...
        dispatch(uiModal({
            modalFor: 'edit_auction',
            modalOpen: true,
            modalTitle: 'Editar Subasta'
        }))
    }
    ...
    return (
        <tr className="hover:bg-gray-100">
            ...
            <td className="p-4 whitespace-nowrap space-x-2 text-center">
                {
                    !(product.products_acutions && product.products_acutions.length > 0)
                    &&
                    <button
                        className=" text-blue-600 hover:text-blue-700 focus:ring-4 focus:ring-cyan-200 font-medium rounded-lg text-sm inline-flex items-center px-1 py-1 text-center"
