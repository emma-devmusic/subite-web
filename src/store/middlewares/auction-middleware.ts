    ...
    return (next: Dispatch) => async (action: PayloadAction<any>) => {
        ...
        if(action.type === 'auction/updateAuction') {
            ...
            try {
                ...
                state.dispatch(uiCloseModal())
                state.dispatch(getProducts('search?page=1&limit=10&with_auction=' + getParamsAuctionState(location.pathname)))
            } catch (error: any) {
                Swal.fire('Error', `${error.code}`, 'error')
            }
            state.dispatch(uiSetLoading(false))
        ...
        if(action.type === 'auction/deleteAuction') {
            ...
            try {
                ...
                state.dispatch(uiCloseModal())
                state.dispatch(getProducts(`search?page=1&limit=10&with_auction=${action.payload.auctionTab}`))
            } catch (error: any) {
                Swal.fire('Error', `${error.code}`, 'error')
            }
            state.dispatch(uiSetLoading(false))
